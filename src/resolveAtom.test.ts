  ...
  describe('when providing the wrapped custom property', () => {
    ...
    describe('with default value parameter', () => {
      it('returns only the custom property', () => {
        ...
        expect(resolveAtom('var(--test-b, rgb(123 123 123))')).toBe(
          'rgb(123 123 123)',
        );
      });
      describe('as custom property', () => {
        ...
        it('returns only the custom property', () => {
          ...
          expect(resolveAtom('var(--test-c, var(--test-b, 16px))')).toBe(
            '16px',
          );
          expect(resolveAtom('var(--test-c, var(--test-b, #fffFFF))')).toBe(
            '#fffFFF',
          );
          expect(resolveAtom('var(--test-b, var(--test-b, 123 123 123))')).toBe(
            '123 123 123',
          );
          expect(
            resolveAtom('var(--test-b, var(--test-b, rgb(123 123 123)))'),
